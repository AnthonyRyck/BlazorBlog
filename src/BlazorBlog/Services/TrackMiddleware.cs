...
namespace BlazorBlog.Middlewares
{
    public class TrackMiddleware
    {
        ...
        public async Task Invoke(HttpContext httpContext, IHttpContextAccessor accessor)
        {
            ...
            File.AppendAllText(@"C:\123\httpContextRequestHeader.txt", "KEY : RemoteIpAddress - " + ip + Environment.NewLine);
            ...
            File.AppendAllText(@"C:\123\httpContextRequestHeader.txt", "KEY : Referer - " + referer + Environment.NewLine);
            
            var deviceModel = httpContext.Request.Headers["sec-ch-ua-model"].ToString();
            File.AppendAllText(@"C:\123\httpContextRequestHeader.txt", "KEY : sec-ch-ua-model - " + deviceModel + Environment.NewLine);

            var Origin = httpContext.Request.Headers["Origin"].ToString();
            File.AppendAllText(@"C:\123\httpContextRequestHeader.txt", "KEY : Origin - " + Origin + Environment.NewLine);
            File.AppendAllText(@"C:\123\httpContextRequestHeader.txt", "-----------------------------" + Environment.NewLine);
